...
public class FurnitureDAOImpl implements FurnitureDAO {
	...
	public int getSumOfOptionDaysForAUserAboutAFurniture(int idFurniture, int idUser) {
		int number = -1;
		try {
			String sql = "SELECT SUM(option_term) FROM pae.options WHERE id_furniture = ? AND id_user = ?";
			ps = dalBackendService.getPreparedStatement(sql);
	...
	private FurnitureDTO setFurniture(ResultSet rs, FurnitureDTO furniture) {
		try {
			...
			furniture.setPickUpDate(rs.getTimestamp(5) == null ? null : rs.getTimestamp(5).toLocalDateTime());
			furniture.setStoreDeposit(rs.getBoolean(6));
			furniture.setDepositDate(rs.getTimestamp(7) == null ? null : rs.getTimestamp(7).toLocalDateTime());
	...
	public List<FurnitureDTO> getFurnitureList() {
		...
		try {
			String sql = "SELECT id_furniture, condition, description, purchase_price, "
					...
					+ "offered_selling_price, id_type, request_visit, seller, favorite_photo " + "FROM pae.furnitures;";
			ps = dalBackendService.getPreparedStatement(sql);
			ResultSet rs = ps.executeQuery();
			FurnitureDTO furniture = null;
			while (rs.next()) {
	...
	@Override
	public void introduceRequestForVisite(String timeSlot, Address address, Map<Integer, List<String>> furnitures) {
	...
	public void cancelOvertimedOptions() {
		try {
			String sql = "UPDATE pae.options" + " SET condition = ?, cancellation_reason = 'Temps dépassé'"
			...
			if (val > 0) {
				sql = "UPDATE pae.furnitures" + " SET condition = ?" + " WHERE condition = ? AND id_furniture NOT IN "
