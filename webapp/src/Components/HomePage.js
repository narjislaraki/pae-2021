...
const HomePage = async (pageData) => {
  ...
  if (!pageData || pageData.idType == "all") {
    ...
    pageData = {};
    pageData.idType="all";
    pageData.titlePage = "Accueil";
  }
  else{
    try{
      furnitures = await callAPI(
        API_BASE_URL + "/" + pageData.idType,
        "GET",
        undefined,
        undefined,
      )
    }catch(err){
      console.error("Homepage::get list by type", err);
      PrintError(err);
    }
    if (furnitures.length == 0){
      let err = {
        message: "Aucun meuble de ce type actuellement"
      }
      PrintError(err);
      RedirectUrl("/");
      return;
...
<div class="homepageTitle">
  <div class="all-furn-title small-caps" id="title">${pageData.titlePage}</div>
  <div class="dropdown">
...
`;
    ...
    for (let index = 0; index < typesOfFurnituresList.length; index++) {
      Array.from(typesOfFurnituresList)[index].addEventListener("click", (e) => {
        e.preventDefault();
        RedirectUrl("/", {idType: typesOfFurnituresList[index].getAttribute("data-id"),
        titlePage: typesOfFurnituresList[index].innerText});
      });
    }
